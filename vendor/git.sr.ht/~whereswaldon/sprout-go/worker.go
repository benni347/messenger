...
func (c *Worker) IngestNode(node forest.Node) error {
	...
	if err := forest.ValidateNode(node, c.SubscribableStore); err != nil {
		...
		for _, ancestor := range ancestry.Nodes {
			...
			if err := forest.ValidateNode(ancestor, c.SubscribableStore); err != nil {
		...
		if err := node.ValidateReferences(c.SubscribableStore); err != nil {
...
func (c *Worker) BootstrapLocalStore(maxCommunities int) {
	...
	for _, node := range communities.Nodes {
		...
		if err := c.IngestNode(community); err != nil {
			c.Printf("Failed ingesting community %s: %v", community.ID().String(), err)
			continue
		}
...
// SynchronizeFullTree ensures that the entire tree rooted at the provided
// node is available within the worker's configured store. maxNodes is the
// number of leaf nodes whose ancestry will be fetched, starting from the
// most recent.
func (c *Worker) SynchronizeFullTree(root forest.Node, maxNodes int, perRequestTimeout time.Duration) error {
	...
	sort.Slice(leafList.Nodes, func(i, j int) bool {
		return leafList.Nodes[i].TreeDepth() < leafList.Nodes[j].TreeDepth()
	})
	for _, leaf := range leafList.Nodes {
		...
		if err := c.IngestNode(leaf); err != nil {
			return fmt.Errorf("failed ingesting leaf node %s: %w", leaf.ID().String(), err)
		}
	}
...
// EnsureAuthorAvailable attempts to ensure that the author of provided node
// is available within the worker's configured store. It returns nil when
// the author is available, and returns error if it is passed invalid
// parameters or if any operations fail.
func (c *Worker) EnsureAuthorAvailable(node forest.Node, perRequestTimeout time.Duration) error {
	...
	if err := forest.ValidateNode(author, c.SubscribableStore); err != nil {
