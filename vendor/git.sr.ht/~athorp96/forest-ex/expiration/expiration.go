...
// IsExpiredTwig returns whether the provided twig data indicates that the
// associated node is expired.
func IsExpiredTwig(td *twig.Data) (bool, error) {
	expired, _, err := ExpirationDataTwig(td)
	return expired, err
}

// ExpirationDataTwig returns whether the provided twig data contains expiration
// data and has expired. See the docs on ExpirationData for details.
func ExpirationDataTwig(td *twig.Data) (hasExpired, canExpire bool, err error) {
	bytes, present := td.Get(TTLKeyName, 1)
	if !present {
...
func ExpirationData(node forest.Node) (hasExpired, canExpire bool, err error) {
	...
	return ExpirationDataTwig(data)
}
