...
var (
	...
	activationKey = filepath.Join("SOFTWARE", "Classes", "CLSID")
)
...
func setAppDataImpl(data AppData) error {
	...
	if data.ActivationExe != "" {
		if err := writeStringValue(
			filepath.Join(activationKey, GUID_ImplNotificationActivationCallback.String(), "LocalServer32"),
			registryDefaultKey,
			data.ActivationExe,
		); err != nil {
